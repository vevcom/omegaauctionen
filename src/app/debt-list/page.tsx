...
export default function DownloadAuctionData() {
    ...
    useEffect(() => {
        async function compile() {
            ...
            if (!compileResponse[0]) {
                alert("noe gikk galt eller du har ikke tilgang" +compileResponse[1].toString())
                return;
            }
    ...
    const downloadAutoGeneratedMailFile = ()=>{
        ...
        if (!auctionData) {
            ...
            blob = new Blob([auctionData[2]], { type: "text/plain" });
        }
        const url = URL.createObjectURL(blob);

        const a = document.createElement("a");
        a.href = url;
        a.download = "mail_liste.txt";
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
